...
const THongKe3 = () => {
    ...
    const fetchData = async () => {
        try {
            // Fetch revenue data
            const revenueResponse = await axiosInstance.get('http://localhost:5030/api/Statistics/revenue-by-date', {
                params: { ngayBatDau: startDate, ngayKetThuc: endDate },
            });

            // Fetch sales data
            const salesResponse = await axiosInstance.get('http://localhost:5030/api/Statistics/topSelling', {
                params: { ngayBatDau: startDate, ngayKetThuc: endDate },
            });
        ...
        <div className={cx('app')}>
            <header className={cx("app-header")}>
                <div className={cx('filters')}>
                    ...
                    <Button variant='add' onClick={fetchData}>Thống kê</Button>
                </div>
            </header>
